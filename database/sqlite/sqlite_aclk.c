...
void after_send_retention(uv_work_t *req, int status)
{
    ...
    (void)status;
    info("Done retention for %s, scheduling a dimension deletion check", wc->node_id);
    stop_retention_run();
...
void send_retention(uv_work_t *req)
{
    struct aclk_database_worker_config *wc = req->data;
...
// NODE OPERATIONS
                ...
                case ACLK_DATABASE_UPD_RETENTION:
                    if (unlikely(wc->retention_running || request_retention_run())) {
                        if (wc->retention_running)
                            info("Retention is running for %s; ignoring request", wc->uuid_str);
                        else
                            info("Retention is running for another node; delaying request");
                        wc->rotation_after = now_realtime_sec() + 60;
                        break;
                    }
                    ...
                    fatal_assert(0 == uv_queue_work(loop, &retention_work, send_retention, after_send_retention));
